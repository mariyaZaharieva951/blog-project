...
const BlogComponent = () => {
  ...
  const [newData, setNewData] = useState<{ id: number; title: string; imageUrl: string; body: string; date: string }[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data: { id: number; title: string; imageUrl: string }[] = [];
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    {newData.map((post, index) => (
      <div key={index} className="post-item overflow-hidden flex flex-col mx-1 lg:mx-0">
        <div className="relative">
          <img src={post.imageUrl} alt={post.title} className="lg:w-[555px] lg:h-[370px] object-cover" />
        </div>
